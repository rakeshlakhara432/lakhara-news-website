    ...
    <div className="space-y-16 pb-32 animate-in fade-in slide-in-from-bottom-5 duration-1000">
      ...
      <section className="text-center space-y-10 pt-12">
         ...
         <div className="space-y-4 px-6 md:px-0">
            <h1 className="text-4xl md:text-5xl font-black text-gray-950 tracking-tighter uppercase italic leading-tight md:leading-tight md:leading-none">लखारा समाज <span className="text-primary underline decoration-primary/10 italic font-black underline-offset-8">विवाह मंच</span></h1>
      ...
      <section className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 px-6 md:px-0">
         <div className="md:col-span-2">
            <div className="bg-white p-4 md:rounded-[4rem] rounded-none border border-gray-100 shadow-bhagva flex flex-col md:flex-row gap-4">
               ...
               <button className="px-10 py-4 bg-primary text-white font-black rounded-full text-[10px] uppercase tracking-widest shadow-xl hover:bg-primary/90 active:scale-95 transition-all outline-none">
                  <Filter className="size-5" /> खोजें
               </button>
            </div>
         </div>
         <div className="bg-white p-4 md:rounded-[4rem] rounded-none border border-gray-100 shadow-sm flex items-center justify-between px-10">
            ...
            <div className="size-12 bg-primary/10 text-primary md:rounded-2xl rounded-none flex items-center justify-center">
      ...
      {isLoading ? (
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-6 md:px-0">
           {filteredProfiles.map((p, i) => (
             <div key={i} className="group p-8 bg-white md:rounded-[4rem] rounded-none border border-gray-100 shadow-sm hover:shadow-bhagva transition-all hover:scale-105 active:scale-95 cursor-pointer relative overflow-hidden">
                ...
                <div className="size-24 mx-auto bg-gray-50 md:rounded-[3rem] rounded-none p-2 shadow-inner group-hover:bg-primary/10 transition-all duration-700 overflow-hidden mb-6 relative border-4 border-white group-hover:border-primary/10">
                   {p.photos?.[0] ? (
                     <img src={p.photos[0]} className="size-full object-cover md:rounded-[2.5rem] rounded-none" alt={p.name} />
                   ) : (
                ...
                <div className="text-center space-y-2">
                   <h3 className="text-lg font-black text-gray-950 tracking-tighter uppercase italic leading-tight md:leading-none truncate">{p.name}</h3>
                ...
                <button className="w-full mt-10 py-4 bg-gray-950 text-white md:rounded-[2rem] rounded-none font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-primary transition-all flex items-center justify-center gap-3">
                   <MessageCircle className="size-4" /> संपर्क करें
                </button>
             </div>
           ))}
           {filteredProfiles.length === 0 && (
             <div className="col-span-full py-20 bg-gray-50 md:rounded-[3rem] rounded-none border-2 border-dashed border-gray-200 text-center">
      ...
      {/* 📢 ADD PROFILE BANNER */}
      <section className="bg-primary/5 border-2 border-primary/10 md:rounded-[5rem] rounded-none p-8 md:p-24 text-center space-y-12 relative overflow-hidden mx-6 md:mx-0">
         ...
         <div className="relative z-10 space-y-10">
            <h2 className="text-3xl md:text-5xl font-black text-gray-950 italic tracking-tighter uppercase leading-tight md:leading-tight md:leading-none">क्या आप अपने पुत्र या पुत्री के लिए <br className="hidden md:block" /> <span className="text-primary animate-pulse underline italic">सुयोग्य जीवनसाथी</span> खोज रहे हैं?</h2>
            ...
            <div className="pt-10">
               <button onClick={() => navigate("/matrimonial/register")} className="px-16 py-6 bg-primary text-white font-black md:rounded-[3rem] rounded-none hover:bg-primary/90 transition-all text-sm tracking-widest uppercase shadow-2xl flex items-center justify-center gap-4 mx-auto hover:scale-110 active:scale-95 border-none outline-none">
