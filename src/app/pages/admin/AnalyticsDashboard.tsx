  ...
  return (
    <div className="space-y-10 pb-24 animate-in fade-in duration-1000">
      ...
      <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
         <div className="flex items-center gap-6">
            <div className="size-16 bg-slate-950 text-white rounded-2xl flex items-center justify-center shadow-xl">
             ...
             <div>
                <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">Command <span className="text-primary">Analytics</span></h1>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-2">Real-time Performance Metrics</p>
             </div>
         </div>
         
         <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-6 py-3 bg-slate-50 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all border border-slate-100">
               <Download className="size-4" /> Export Report
            </button>
            <button className="px-6 py-3 bg-primary text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-primary/90 transition-all shadow-xl shadow-primary/20">
               Refresh Data
            </button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: "Total Site Views", val: stats?.totalViews || "0", icon: Eye, color: "text-blue-600 bg-blue-50", trend: "0%" },
            { label: "Community Reviews", val: stats?.newReviews || "0", icon: Star, color: "text-amber-600 bg-amber-50", trend: "Neutral" },
            { label: "Active Members", val: stats?.activeUsers || "0", icon: Users, color: "text-primary bg-primary/5", trend: "0%" },
            { label: "Approval Queue", val: "0", icon: CheckCircle, color: "text-emerald-600 bg-emerald-50", trend: "Clear" }
          ].map((card, i) => (
            ...
            <div className="h-[300px] w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                     ...
                     />
                     <Bar dataKey="value" fill="var(--primary)" radius={[10, 10, 10, 10]} barSize={40} />
                  </BarChart>
               </ResponsiveContainer>
            </div>
         </div>
      ...
      <div className="bg-slate-950 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { label: "Site Shield", icon: ShieldCheck, color: "bg-slate-800", desc: "Security Firewall" },
              { label: "Auto Approve", icon: CheckCircle, color: "bg-emerald-600", desc: "System Logic" },
              { label: "Global Sync", icon: Activity, color: "bg-primary", desc: "Database Bridge" },
